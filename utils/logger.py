...
class Logger:
    def __init__(self, log_dir, log_file_name='log.txt'):
        self.log_dir = log_dir
        self.log_file_name = log_file_name
        self.log_file = os.path.join(log_dir, log_file_name)

    def log_loss(self, loss_dict, other_dict):
    ...
    def log_img(self, img_tensor, epoch=None, iter=None):
        if epoch is not None:
            img_name = 'epoch-' + str(epoch).zfill(4) + '.png'
            img_path = os.path.join(self.log_dir, img_name)
        elif iter is not None:
            img_name = 'iter-' + str(iter).zfill(8) + '.png'
            img_path = os.path.join(self.log_dir, img_name)
        else:
            raise Exception('args <epoch> or <iter> needed.')
    ...
    def log_ckpt(self, model, epoch):
        model_name = 'model-' + str(epoch).zfill(4) + '.pth'
